...
var _ = Describe("GPU Allocator", func() {
	...
	Context("Dedicated GPU Allocation", func() {
		It("should allocate dedicated GPU with full capacity", func() {
			// Use small resource request but expect full GPU capacity to be allocated
			request := tfv1.Resource{
				Tflops: resource.MustParse("10"),
				Vram:   resource.MustParse("2Gi"),
			}

			// Create dedicated GPU allocation request
			mutex.Lock()
			gpus, err := allocator.Alloc(&tfv1.AllocRequest{
				PoolName:              "test-pool",
				WorkloadNameNamespace: workloadNameNs,
			...
			// Verify that entire GPU capacity is allocated (available should be 0)
			allocatedGPU := gpus[0]
			Expect(allocatedGPU.Status.Available.Tflops.Value()).To(Equal(int64(0)))
			Expect(allocatedGPU.Status.Available.Vram.Value()).To(Equal(int64(0)))

			// Cleanup
			deallocateAndSync(gpus)
		})

		It("should restore full GPU capacity on dedicated GPU deallocation", func() {
			request := tfv1.Resource{
				Tflops: resource.MustParse("10"),
				Vram:   resource.MustParse("2Gi"),
			}

			// Get original GPU state
			originalGPU := getGPU("gpu-1")
			originalTflops := originalGPU.Status.Capacity.Tflops.DeepCopy()
			originalVram := originalGPU.Status.Capacity.Vram.DeepCopy()
			...
			// Verify entire capacity is allocated
			allocatedGPU := gpus[0]
			Expect(allocatedGPU.Status.Available.Tflops.Value()).To(Equal(int64(0)))
			Expect(allocatedGPU.Status.Available.Vram.Value()).To(Equal(int64(0)))
			...
			// Verify full capacity is restored
			restoredGPU := getGPU(allocatedGPU.Name)
			Expect(restoredGPU.Status.Available.Tflops.Equal(originalTflops)).To(BeTrue())
			Expect(restoredGPU.Status.Available.Vram.Equal(originalVram)).To(BeTrue())
		})
	})
